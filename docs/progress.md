...
- **Bug found & fixed: front desk got 403 adding to queue.** Generating the visit *queue number* saves the visit → `VisitValidator` lists encounters → needs `Get Encounters`, which Front Desk deliberately lacks (1.6 clinical-data exclusion, Test Gate 1). The queue *entry* itself needs no such privilege. Fix: `visitQueueNumberAttributeUuid: null` for patient-chart + service-queues apps — queue numbers off rather than weakening the privilege boundary. Revisit if a clinic wants call-by-number (would need an upstream queue-module change to skip visit re-validation).
- **Providers required to prescribe:** `create-dev-users.sh` now also creates Provider records for admin + clinician (the O3 drug-order workspace crashes on `currentProvider.uuid` without one).
- Quirks noted for later: registration accepts an uncommitted DOB string and fails server-side (add client validation); patient-banner "Move patient to next service" crashes (`undefined.allowedPriorities`) while the queue board's Transfer works — use/train the board path; queue board "View: All" shows nothing (must pick a location explicitly); e2e suite re-verified green after the queue-number change (11 passed).
...
- Demo cleanout: the demo drug files (320 drugs + 469 random-uuid drug concepts whose names collide with CIEL FSNs) are **deleted from the build** in distro/pom.xml — retire-overlays proved non-deterministic because Initializer's intra-domain file order doesn't guarantee retires run after the base file creates its targets (first CI clean boot shipped 744 drugs). Verified with `make destroy && make dev` + full suite on the empty DB, then CI.
